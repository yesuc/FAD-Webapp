...
def find_menu_page(home_url):
    ...
    key_words = ['Menu','Food', 'Dining', 'Dinner','Lunch', 'Sandwich', 'Breakfast', 'Soups', 'Pastries']
...
# RETURN: A list of string urls to perspective menus
    ...
    while i < len(key_words):
        try:
            ...
            while j < len(link_matches):
                link = link_matches[j]
                link_href = link.get_attribute('href')
                print(link.text, link_href, link.is_displayed(), type(link_href))
                if link_href is not None:
                    for key in ['pdf','download']:
                        if key in link_href:
                            pdf_urls.append(link_href)
                j+=1
        except:  #catch error or exception
            pass
        i+=1
