...
#RETURNS: an array as a json string
  ...
  # scope :allergen_free, -> (allergen){ joins(:restaurant).group(:restaurant_id).having("contains_#{allergen}".to_sym => false) }
  scope :gluten_free, -> { where contains_gluten: false }
  scope :dairy_free, -> { joins(:restaurant).where contains_dairy: false}
